          ...
          <validation-observer v-slot="{ invalid }">
            <form ref="form" @submit.prevent="handleOtp">
              <div class="register-head">
                <h1 class="display-1 font-weight-bold">Daftar Agreesip</h1>
...
<script>
import { reactive, useRouter, defineComponent, useStore, useRoute, onMounted } from '@nuxtjs/composition-api'
export default defineComponent({
  ...
  setup() {
    ...
    onMounted(() => {
      register.referralCode = route.value.query.referral_code
    })

    const handleOtp = () => {
      store.dispatch('referral/register', register).then(() => {
